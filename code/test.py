...
# 建立分词器
tokenizer1 = BertTokenizer.from_pretrained('pretrain_models/bert-base-uncased')
# 不使用分词器对英语单词进行分词
def collate_fn(batch):
    ...
    for i, (text, text_labels) in enumerate(batch):
        token_ids = tokenizer1.encode_plus(text, add_special_tokens = True, max_length=maxlen, truncation=True)["input_ids"]
...
class Model(BaseModel):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=768, hidden_size=hidden_size//2, num_layers=num_layers, bidirectional=True, batch_first=True)
...
def evaluate(data, threshold=-1.5, result_label_file=""):
    result_file = open(result_label_file, "w+")
    ...
    result_file.close()
    return tp_sk, tpfp_sk, tpfn_sk, support_sk, tp_ck, tpfp_ck, tpfn_ck, support_ck
...
def search_best_tr(trs, test_dataloader, result_label_file_path):
    best_tr = 0.0
    best_micro_f1 = 0.0
    for tr in trs:
        tp_sk, tpfp_sk, tpfn_sk, support_sk, tp_ck, tpfp_ck, tpfn_ck, support_ck = evaluate(test_dataloader, tr, result_label_file_path)
        ...
        if micro_f1 > best_micro_f1:
            ...
            best_tr = tr
            print("best_micro_f1:{:.4f}, best_tr:{:.2f}".format(best_micro_f1, best_tr))
            ...
            print("BEST RESULT".center(50, "#"))
            print("best_micro_f1:{:.4f}, best_tr:{}".format(best_micro_f1, best_tr))
            evaluate(test_dataloader, best_tr, result_label_file_path)
            break
    return best_tr, best_micro_f1


if __name__ == '__main__':
    model.load_weights("checkpoints/paecrf"+str(hidden_size)+str(num_layers)+".pt")
    print("hidden_size={}, num_layers={}".format(str(hidden_size), str(num_layers)))
    ...
    for dataset in dataset_directorys:
        for model_name in model_names:
            test_file = "test.txt"
            result_label_file = "result_label.txt"
            best_param_file = "param.txt"
            dataset_path = os.path.join(dir, dataset, test_file)
            result_label_file_path = os.path.join(dir, dataset, model_name, result_label_file)
            best_param_file_path = os.path.join(dir, dataset, model_name, best_param_file)
            data = MyDataset(dataset_path)
            test_dataloader = DataLoader(data, batch_size=batch_size, collate_fn=collate_fn)

            # 搜索最佳tr
            mintr = -5.0, maxtr = 0, sep = 0.5
            trs = np.arange(mintr, maxtr, sep, dtype=float)
            best_tr, best_f1 = search_best_tr(trs, test_dataloader, result_label_file_path)
            mintr = best_tr - 0.5, maxtr = best_tr + 0.5, sep = 0.1
            trs = np.arange(mintr, maxtr, sep, dtype=float)
            best_tr, best_f1 = search_best_tr(trs, test_dataloader, result_label_file_path)
